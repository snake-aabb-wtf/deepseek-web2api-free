...
- **Upstream rate-limit adaptation**: automatically backs off and retries when triggering an upstream rate limit (429) (`DEEPSEEK_RATE_LIMIT_RETRY_DELAYS`, defaults to `5,15`, rotating to a new session each time); `DEEPSEEK_JITTER_SECS` default changed to `0.4`, reducing the chance of hitting rate limits.
- **hif signature headers**: automatically fetches `hif-leim/hif-dliq.deepseek.com/query` and sends `x-hif-leim`/`x-hif-dliq` (TTL cache 600s; silently degrades on fetch failure without affecting requests).
- **Test coverage**: added `tests/test_adapter.py` (15 cases) and `tests/test_account_pool.py` (10 cases), for a full suite of 81 tests.
- Fixed streaming toast/hint error detection (upstream error `data` is top-level JSON rather than a `v` wrapper).

---

## v3.2.1 Fixes

Patch release 2026-08-03 — **located and fixed the root cause of empty streaming/non-streaming responses**:

- **Swallowed upstream rate-limit hints**: when requests are too frequent, chat.deepseek.com returned HTTP 200 + SSE `event: hint` (`finish_reason=rate_limit_reached`, "You are sending messages too frequently, please try again later") with no content tokens; the adapter previously did not parse `hint` events and treated the rate limit as "empty content".
- **Fix**: added `RateLimitError`/`UpstreamHintError` hint parsing (streaming + non-streaming); `/v1/chat/completions` now returns **HTTP 429** with a clear error on rate limit (instead of silently returning 200 with empty content); other hint errors return 502.
- Practical advice: low-frequency requests (`DEEPSEEK_JITTER_SECS=0.4`) or spreading across multiple accounts to avoid triggering upstream rate limits.
- See [docs/release-notes/v3.2.1.md](docs/release-notes/v3.2.1.md).

---

## What's New in v3.2.0

Maintenance release 2026-08 (based on v3.0.0), mainly:

- **start.bat one-click startup refactor**: auto-detects/creates the virtual environment (`.venv`/`venv`/`env`), validates and auto-installs dependencies, auto-runs `npm` build when WebUI artifacts are missing, prints the access URL before startup and auto-opens the browser after 3 seconds by default (`WEBUI_OPEN=0` to skip); on port conflict only kills this project's own processes (uses PowerShell instead of the deprecated `wmic`).
- **WebUI overhaul**: fixed static-resource 404 blank screen (vite `base: '/webui/'` + react-router `basename`), 7 functional bug fixes and UI polish, Playground rewrite (parameters mapped to DeepSeek's real forwarded fields, removed ineffective temperature slider, reasoning process visualization, quick/expert model modes).
- **Auth**: `/v1/*` now accepts a valid admin session token — after logging in via webui you can call the model list and Playground directly without being logged out.
- **Account pool**: `.env` credentials now act as a read-only fallback when the pool is empty (panel accounts take priority; pool empty no longer returns 503).
- **Upstream empty-response hardening**: when the upstream occasionally returns an empty SSE, automatically retry once with a new session; if still empty, error clearly (non-streaming 502 / streaming error frame) instead of silently returning empty content.
- **Adapter**: `X-Client-Version` aligned with browser (2.3.0), removed outdated `X-App-Version`; reverse-recorded the upstream `x-hif-*` signature header mechanism.
- Full changelog at [docs/release-notes/v3.2.0.md](docs/release-notes/v3.2.0.md).

---

## What's New in v3.0.0

v3.0.0 rewrote the entire admin panel as a **React + shadcn/ui single-page application** (`webui-new/`):

- **Brand-new WebUI** — Vite + React 18 + TypeScript + shadcn/ui, 6 pages (login / overview / account pool / Playground / settings / 404), light/dark theme switching, Recharts live trend charts.
- **New backend endpoints** — `GET /admin/api/history` (request volume / latency time-series over a 30s rolling window), `GET /admin/api/env` (sanitized view of runtime configuration).
- **Build** — run `npm install && npm run build` inside `webui-new/` (Windows can also use `build.bat`), output to `webui-new/dist/`; visiting `/webui` without building returns a JSON build hint.
- API endpoints (`/v1/*`, `/admin/api/*`) remain fully backward compatible; the only breaking change is that the WebUI artifact directory moved from `webui/` to `webui-new/dist/`.

---

## What's New in v2.2.0

v2.2.0 introduced numerous security and feature improvements while maintaining backward compatibility:

- **Security hardening**
  - Default `HOST=127.0.0.1` (no longer defaults to `0.0.0.0`); refuses to start when bound to public + using default password.
  - Structured JSON logs (`LOG_FORMAT=json`), switchable to `text`.
  - Optional Fernet (AES-128-CBC + HMAC) encryption for `data/accounts.json` (`DEEPSEEK_ENCRYPTION_KEY`).
  - Strict CORS (same-origin by default, requires `ALLOWED_ORIGINS` to explicitly allow).
  - XFF only trusted for proxy downstreams within the `TRUSTED_PROXIES` whitelist.
  - All responses inject `X-Content-Type-Options` / `X-Frame-Options` / `Referrer-Policy` security headers.
  - WebUI additionally injects a `Content-Security-Policy` header.
- **Bug fixes**
  - Anthropic non-streaming endpoint no longer drops `thinking` content.
  - Account error state auto-recovers in background (once every 3 failures).
  - `usage` field returns real token counts (based on tiktoken cl100k_base; falls back to a character heuristic when tiktoken is unavailable).
  - StreamSieve boundaries: iterative draining, `<` boundary, 1 MB buffer cap (configurable).
- **New features**
  - **Model routing** (`MODEL_ROUTES`) — switches quick/expert based on the `model` field.
  - **Multi-turn sessions** (`SESSION_CACHE_TTL`) — reuses a `chat_session_id` for the same user/session.
  - **Client rate limiting** (`CLIENT_RPM_PER_KEY` / `CLIENT_RPM_PER_IP`) — dual-dimension sliding window.
  - **P50/P95/P99 latency** + **success rate** exposed at `/admin/api/stats`.
- **Engineering**
  - 56 pytest unit tests, CI matrix (Python 3.10 / 3.11 / 3.12).
  - All silent passes (`except Exception: pass`) replaced with `logger.exception(...)`.
  - All `print(WARNING...)` replaced with structured logging.
